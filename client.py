...
ap.add_argument(
    "action", help="What to do, can be: login, register, profile, upload, delete, get"
)
...
if args.action in ("login", "register"):  # pretty much the same approach.
    ...
    r = requests.post(
        f"{args.baseurl}/me",
        headers={"Authorization": "Bearer " + os.environ.get("CLITOKEN", "")},
    )
    ...
    with open(args.body.get("data", ""), "rb") as f:
        data = f.read()
    r = requests.post(
        f"{args.baseurl}/up/{args.body.get("format","txt")}/{args.body.get("filename",random.choices("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890"))}",
        headers={
            "Content-Type": "application/json",
            "Authorization": "Bearer " + os.environ.get("CLITOKEN", ""),
    ...
    r = requests.post(
        f"{args.baseurl}/del/{args.body.get("filename")}",
        headers={
            "Content-Type": "application/json",
            "Authorization": "Bearer " + os.environ.get("CLITOKEN", ""),
        },
    ...
    # filename: str
    r = requests.get(
        f"{args.baseurl}/files/{args.body.get("id",0)}/{args.body.get("filename","")}"
    )
    print(r.content)
